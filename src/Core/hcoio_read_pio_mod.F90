...
!
  ...
  ! PIO cached file descriptor is stored in HcoState%ReadLists%FilesPioFh
  ! and HcoState%ReadLists%FilesPioOpen (defined in hco_types_mod.F90).
...
!
    ...
    IF ( RC /= HCO_SUCCESS ) THEN
       MSG = 'Error encountered in routine "SrcFile_Parse", located '     // &
             'module src/Core/hcoio_read_pio_mod.F90!'
 ...
 310            FORMAT( 'HEMCO: OPTIONAL FILE NOT FOUND ', a )
    ...
    ! (PIO version: use HcoState%ReadLists%FilesPioFh instead of FileLun)
    ncLun_valid = .FALSE.
    IF ( HcoState%ReadLists%FilesPioOpen ) THEN
       IF ( TRIM(HcoState%ReadLists%FileInArchive) == TRIM(srcFile) ) THEN
          ncLun = HcoState%ReadLists%FilesPioFh
          ncLun_valid = .TRUE.
       ELSE
          CALL NC_CLOSE ( HcoState%ReadLists%FilesPioFh )
          HcoState%ReadLists%FilesPioOpen = .FALSE.
...
100    FORMAT( 'HEMCO: Opening ', a )
       ...
       HcoState%ReadLists%FileInArchive = TRIM(srcFile)
       HcoState%ReadLists%FilesPioFh = ncLun
       HcoState%ReadLists%FilesPioOpen = .TRUE.
