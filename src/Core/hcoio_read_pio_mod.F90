...
100    FORMAT( 'HEMCO: Opening ', a )
    ...
    !-----------------------------------------------------------------
    ! Cache-aware data read (PIO mirror of hcoio_read_std_mod.F90).
    !
    ! For HCO_CFLAG_INTER fields, split the classic two-slice bracket
    ! read into two single-slice reads and cache the raw slices on
    ! FileData. Subsequent refreshes can then
    !   (a) skip all disk I/O and just re-blend from cache when the
    !       bracket is unchanged; or
    ...
    ! wgt1/wgt2 returned by GET_TIMEIDX, which also fixes the stale-
    ! weight issue inherent to the previous implementation (weights
    ! were frozen into V2/V3 at read time).
    !
    ! Blends apply an OR-mask: pixels where either cached slice is
    ! HCO_MISSVAL are marked missing in the output rather than blended
    ! linearly, preventing fractional-weight blends of a fill value
    ! with a real value from leaking garbage past the downstream
    ! '== HCO_MISSVAL' check (single-side-fill bug).
    !
    ...
    ELSEIF ( isInterBlend .AND. cacheHitSlide ) THEN
       ...
       CALL NC_READ_ARR( fID     = ncLun,              &
                         ...
                         RC      = NCRC                 )
       IF ( NCRC /= 0 ) THEN
          CALL HCO_ERROR( 'NC_READ_ARRAY (I-slide)', RC )
          RETURN
       ENDIF
    ...
    ELSEIF ( isInterBlend ) THEN
       ...
       CALL NC_READ_ARR( fID     = ncLun,              &
                         ...
                         RC      = NCRC                 )
       IF ( NCRC /= 0 ) THEN
          CALL HCO_ERROR( 'NC_READ_ARRAY (I-lower)', RC )
          RETURN
       ENDIF
       ...
       CALL NC_READ_ARR( fID     = ncLun,              &
                         ...
                         RC      = NCRC                 )
       IF ( NCRC /= 0 ) THEN
          CALL HCO_ERROR( 'NC_READ_ARRAY (I-upper)', RC )
          RETURN
       ENDIF
       ALLOCATE( Lct%Dct%Dta%CacheSlice2(                             &
                    SIZE(ncArr,1), SIZE(ncArr,2),                     &
