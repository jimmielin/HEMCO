...
! netCDF files using PIO (Parallel I/O) for CESM. This is a PIO replacement
! for the read-related functions in hco\_ncdf\_mod.F90.
!\\
!\\
! !INTERFACE:
!
...
!
    ...
    IF (tmp_ierr /= PIO_NOERR) THEN
       hasVar = .FALSE.
    ELSE
       tmp_ierr = pio_inq_att(fID, tmp_varid, TRIM(a_name), a_type)
       hasVar = (tmp_ierr == PIO_NOERR)
    ENDIF
    IF ( .NOT. hasVar ) THEN
       varUnit = ''
...
!
    ...
    IF ( YEAR > 0 ) THEN
       ...
       IF ( minDiff /= 0 ) THEN
          PRINT *, 'In HCO_PIO_MOD: Time stamp not found ' // &
...
!
    ...
    ! Init
    LOC = 'TIMEUNIT_CHECK (hco_pio_mod.F90)'
